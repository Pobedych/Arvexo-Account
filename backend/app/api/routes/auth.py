...
from app.core.rate_limit import rate_limit_login, rate_limit_refresh, rate_limit_register
...
def refresh(
    ...
    db: DBSession = Depends(get_db),
    _rl: None = Depends(rate_limit_refresh),
) -> RefreshResponse:
...
def delete_account(
    ...
    write_audit(db, "delete_account", user_id=current_user.id)
    db.flush()
